...
class FlashWorker {
 ...
 public:
  ...
  void Init(const grape::CommSpec& comm_spec,
            const grape::ParallelEngineSpec& pe_spec = grape::DefaultParallelEngineSpec()) {
    graph_->PrepareToRunApp(comm_spec_, prepare_conf_);
    comm_spec_ = comm_spec;
    MPI_Barrier(comm_spec_.comm());

    fw_ = new FlashWare<fragment_t, value_t>;
 ...
 private:
  ...
  FlashWare<fragment_t, value_t>* fw_;
