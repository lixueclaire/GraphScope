...
template <typename VDATA_T, typename EDATA_T>
class FragmentWrapper<DynamicProjectedFragmentPoc<VDATA_T, EDATA_T>>
    : public IFragmentWrapper {
  using fragment_t = DynamicProjectedFragmentPoc<VDATA_T, EDATA_T>;
 ...
 public:
  FragmentWrapper(const std::string& id, rpc::graph::GraphDefPb graph_def,
      ...
      : IFragmentWrapper(id),
        graph_def_(std::move(graph_def)),
        fragment_(std::move(fragment)) {
    CHECK_EQ(graph_def_.graph_type(), rpc::graph::DYNAMIC_PROJECTED_POC);
  }
