...
#ifndef ANALYTICAL_ENGINE_CORE_FRAGMENT_DYNAMIC_PROJECTED_FRAGMENT_POC_H_
#define ANALYTICAL_ENGINE_CORE_FRAGMENT_DYNAMIC_PROJECTED_FRAGMENT_POC_H_
...
class AdjList {
 ...
 public:
  ...
  iterator begin() {
    return iterator(begin_, prop_key_);
  }

  iterator end() {
    return iterator(end_, prop_key_);
  }

  iterator begin() const {
    return const_iterator(begin_, prop_key_);
  }

  iterator end() const {
    return const_iterator(end_, prop_key_);
  }
...
class DynamicProjectedFragmentPoc {
 public:
  ...
  DynamicProjectedFragmentPoc(fragment_t* frag, const std::string& v_prop_key,
  ...
  inline adj_list_t GetIncomingInnerVertexAdjList(const vertex_t& v) {
    // TODO(weibin): fullfill these method.
    return adj_list_t();
  }
