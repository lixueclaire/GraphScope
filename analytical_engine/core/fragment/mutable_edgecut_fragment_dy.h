...
#ifndef GRAPE_FRAGMENT_MUTABLE_EDGECUT_FRAGMENT_POC_H_
#define GRAPE_FRAGMENT_MUTABLE_EDGECUT_FRAGMENT_POC_H_
...
struct MutableEdgecutFragmentTraitsDy {
  // using OID_T = int64_t;
...
class MutableEdgecutFragmentDy
    : public CSREdgecutFragmentBase<
          // OID_T, VID_T, VDATA_T, EDATA_T,
	  gs::dynamic::Value, vineyard::property_graph_types::VID_TYPE, gs::dynamic::Value, grape::EmptyType,
          MutableEdgecutFragmentTraitsDy> {
 public:
  ...
  using VDATA_T = gs::dynamic::Value;;
  using EDATA_T = grape::EmptyType;
  using traits_t = MutableEdgecutFragmentTraitsDy;
  using base_t =
  ...
  MutableEdgecutFragmentDy()
      : FragmentBase<OID_T, VID_T, VDATA_T, EDATA_T, traits_t>(NULL) {}
  explicit MutableEdgecutFragmentDy(std::shared_ptr<vertex_map_t> vm_ptr)
      : FragmentBase<OID_T, VID_T, VDATA_T, EDATA_T, traits_t>(vm_ptr) {}
  virtual ~MutableEdgecutFragmentDy() = default;
  ...
  void Init(fid_t fid, bool directed, std::vector<internal_vertex_t>& vertices,
            ...
            LoadStrategy load_strategy_2 = LoadStrategy::kOnlyOut) override {
    init(fid, directed);

    ovnum_ = 0;
    load_strategy_ = load_strategy_2;
    ...
    LOG(INFO) << "dy mutable fragment iterating, edge_num=" << oe_.edge_num();
    double t = grape::GetCurrentTime();
    for (auto& v : InnerVertices()) {
      // auto es = adj_list_t(oe_.get_begin(v), oe_.get_end(v));
      auto es = this->GetOutgoingAdjList(v);
      for (auto& e : es) {
        auto n = e.get_neighbor();
      }
    }
    LOG(INFO) << "dy mutable iterating time: " << grape::GetCurrentTime() - t;
  ...
  void Mutate(Mutation<vid_t, vdata_t, edata_t>& mutation) {
    ...
    LOG(INFO) << "mutable fragment iterating, edge_num=" << oe_.edge_num();
    ...
    LOG(INFO) << "mutable iterating time: " << GetCurrentTime() - t;
  }

  void MutateDynamic(Mutation<vid_t, vdata_t, edata_t>& mutation) {
    vertex_t v;
    if (static_cast<double>(mutation.vertices_to_remove.size()) /
            static_cast<double>(this->GetVerticesNum()) <
        0.1) {
