...
#ifndef ANALYTICAL_ENGINE_CORE_LOADER_ARROW_TO_DYNAMIC_CONVERTER_POC_H_
#define ANALYTICAL_ENGINE_CORE_LOADER_ARROW_TO_DYNAMIC_CONVERTER_POC_H_
 ...
 */
template <>
struct DynamicWrapperPoc<int64_t> {
 ...
 */
template <>
struct DynamicWrapperPoc<std::string> {
...
class ArrowToDynamicConverterPoc {
 ...
 public:
  explicit ArrowToDynamicConverterPoc(const grape::CommSpec& comm_spec,
 ...
 private:
      ...
      for (fid_t fid = 0; fid < fnum; fid++) {
        for (vid_t offset = 0;
          ...
          if (v_label == default_label_id_) {
            DynamicWrapperPoc<oid_t>::to_dynamic(oid, to_oid);
            dst_vm_ptr->AddVertex(to_oid, gid);
          } else {
            DynamicWrapperPoc<oid_t>::to_dynamic_array(label_name, oid, to_oid);
            dst_vm_ptr->AddVertex(to_oid, gid);
          }
        }
      }
  ...
  bl::result<std::shared_ptr<dst_fragment_t>> convertFragment(
    ...
    // TODO(weibin): multi-thread
    auto fid = src_frag->fid();
    const auto& schema = src_frag->schema();
    std::vector<typename dst_fragment_t::internal_vertex_t> processed_vertices;
    std::vector<typename dst_fragment_t::edge_t> processed_edges;
    ...
    dynamic_frag->Init(src_frag->fid(), src_frag->directed(), processed_vertices, processed_edges);
    return dynamic_frag;
  }
