...
#    cd /root/gs/coordinator/grpc_gateway && \
#    go mod download && \
#    make install-tools && \
#    make gateway && \
#    cp bin/gateway /usr/local/bin/graphscope-gateway

RUN cp /root/gs/coordinator/grpc_gateway/bin/gateway /usr/local/bin/graphscope-gateway
