    ...
    **Reporting:**
    ...
    def get_node_data(self, n):
        ...
        op = dag_utils.report_graph(
            self, types_pb2.NODE_DATA, node=json.dumps(n)
        )
        return op.eval()
    ...
    def has_node(self, n):
        ...
        try:
            if self.graph_type == graph_def_pb2.ARROW_PROPERTY:
                n = self._convert_to_label_id_tuple(n)
            op = dag_utils.report_graph(
                self, types_pb2.HAS_NODE, node=json.dumps(n)
            )
    ...
    def _get_nbrs(self, n, report_type=types_pb2.SUCCS_BY_NODE):
        ...
        if self.graph_type == graph_def_pb2.ARROW_PROPERTY:
            n = self._convert_to_label_id_tuple(n)
        op = dag_utils.report_graph(
            self, report_type, node=json.dumps(n)
        )
        ret = op.eval()
        print("get nbrs:", ret)
        return ret
    ...
    def _get_degree(self, n, weight=None, report_type=types_pb2.OUT_DEG_BY_NODE):
        ...
        """
        op = dag_utils.report_graph(
            self, report_type, node=json.dumps(n), key=weight
        )
    ...
    def _clear_adding_cache(self):
        if self._add_node_cache:
            nodes_to_modify = json.dumps(self._add_node_cache, option=json.OPT_SERIALIZE_NUMPY)
        ...
        if self._add_edge_cache:
            edges_to_modify = json.dumps(self._add_edge_cache, option=json.OPT_SERIALIZE_NUMPY)
    ...
    def _clear_removing_cache(self):
        if self._remove_node_cache:
            nodes_to_modify = json.dumps(self._remove_node_cache, option=json.OPT_SERIALIZE_NUMPY)
            self._op = dag_utils.modify_vertices(
        ...
        if self._remove_edge_cache:
            edges_to_modify = json.dumps(self._remove_edge_cache, option=json.OPT_SERIALIZE_NUMPY)
            self._op = dag_utils.modify_edges(
